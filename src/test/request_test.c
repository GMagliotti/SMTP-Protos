...
START_TEST(test_request_connect_domain)
{
	...
	request_parser_init(&parser);
	uint8_t data[] = {
		0x05, 0x01, 0x00, 0x03, 0x0f, 0x77, 0x77, 0x77, 0x2e, 0x69, 0x74,
		0x62, 0x61, 0x2e, 0x65, 0x64, 0x75, 0x2e, 0x61, 0x72, 0x00, 0x50,
	};
	buffer b;
	FIXBUF(b, data);
	bool errored = false;
...
START_TEST(test_request_connect_ipv4)
{
	...
	uint8_t data[] = { 0x05, 0x01, 0x00, 0x01, 0x7f, 0x00, 0x00, 0x01, 0x23, 0x82 };
	buffer b;
	FIXBUF(b, data);
	bool errored = false;
	enum request_state st = request_consume(&b, &parser, &errored);
...
START_TEST(test_request_connect_ipv6)
{
	...
	uint8_t data[] = { 0x05, 0x01, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		               0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x23, 0x82 };
	buffer b;
	FIXBUF(b, data);
	bool errored = false;
	enum request_state st = request_consume(&b, &parser, &errored);
