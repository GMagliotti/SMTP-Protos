...
int
parser_feed(struct selector_key* key, parser_definition* parser, unsigned current_state_id, uint8_t c)
{
	if (parser == NULL) {
	...
	// Buscamos si hay alguna transicion que acepte el caracter, para eso primero obtenemos todas las transiciones del
	// estado actual
...
void
add_accepted_chars_to_transition(parser_transition* transition, uint8_t* array_chars)
{
...
void
add_rejected_chars_to_transition(parser_transition* transition, u_int8_t* array_chars)
{
